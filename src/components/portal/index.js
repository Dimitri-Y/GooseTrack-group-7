...
// };
const createContainer = (containerOptions) => {
  if (document.getElementById(containerOptions.id)) {
    return;
  }

  const { id, mountNode = document.body } = containerOptions;
...
// };
const Portal = (PortalProps) => {
  const { id, children } = PortalProps;
